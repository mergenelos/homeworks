...
%% Calculate Phi
N = length(u);
phi = [-y(4:N-1)' -y(3:N-2)' -y(2:N-3)' -y(1:N-4)' u(4:N-1)' u(3:N-2)' u(2:N-3)' u(1:N-4)'];
%% Calculate Theta_hat
Y = y(5:N)';
...
y_hat = phi*theta_hat;
t=1:length(y_hat);
figure;plot(t,yWithoutNoise(:,1:length(y_hat)),t,y_hat);fontsize( 24 ,"points");legend('measured','simulated');
grid on;hold on;
title("LS parameter estimation - Overparameterized system");xlabel('Sample Number');ylabel('Amplitude');

%% Recreate system model
sysCSD = tf(theta_hat(5:8)',[1 theta_hat(1:4)'],0.3)
sysCS = d2c(sysCSD)